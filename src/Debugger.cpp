...
void Debugger::enterDebugLoop(MemReaderInterface* mr_intf, IniReaderInterface* ir_intf)
{
	...
	while (1)
	{
		...
		if (userInput.compare(0, 1, "?") == 0)
			...
			scanForString(mr_intf, OT_zonename, 0x1000000, userInput.erase(0, 3));
		else if (userInput.compare(0, 2, "ft") == 0)
			scanForString(mr_intf, OT_target, 0x1000000, userInput.erase(0, 3));
		else if (userInput.compare(0, 2, "fs") == 0)
			scanForString(mr_intf, OT_self, 0x1000000, userInput.erase(0, 3));
			...
			scanForUINTFromSelf(mr_intf, 0x1000, userInput.erase(0, 4));
		else if (userInput.compare(0, 3, "sbt") == 0)
			scanForBYTEFromTarget(mr_intf, 0x1000, userInput.erase(0, 3));
...
void Debugger::displayCurrentOffsets()
{
	...
	{
		cout.width(10);
		cout << setfill(' ') << right << i << ") " << spawnParser.ptrNames[i] << " = 0x" << setw(3) << hex << setfill('0') << spawnParser.offsets[i] << dec << " (" << spawnParser.offsets[i] << ")" << endl;
	}
	...
	{
		cout.width(10);
		cout << setfill(' ') << right << i << ") " << itemParser.offsetNames[i] << " = 0x" << setw(3) << hex << setfill('0') << itemParser.offsets[i] << dec << " (" << itemParser.offsets[i] << ")" << endl;
	}
	...
	{
		cout.width(10);
		cout << setfill(' ') << right << i << ") " << worldParser.offsetNames[i] << " = 0x" << setw(3) << hex << setfill('0') << worldParser.offsets[i] << dec << " (" << worldParser.offsets[i] << ")" << endl;
	}
...
void Debugger::processSpawn(MemReaderInterface* mr_intf, offset_types ot)
{
	...
	cout << " " << ptrNames[ot] << " = 0x" << hex << pMem << endl;
	...
	// level
	cout << "    " << spawnParser.ptrNames[spawnParser.OT_level] << " -> " << dec << (UINT)spawnParser.tempNetBuffer.level << endl;

	DISPLAY_SPAWN_ITEM(OT_race, race);

	// class
	cout << "    " << spawnParser.ptrNames[spawnParser.OT_class] << " -> " << dec << (UINT)spawnParser.tempNetBuffer._class << endl;
	...
	// type
	cout << "    " << spawnParser.ptrNames[spawnParser.OT_type] << " -> " << dec << (UINT)spawnParser.tempNetBuffer.type << endl;

	// hidden
	cout << "    " << spawnParser.ptrNames[spawnParser.OT_hidden] << " -> " << dec << (UINT)spawnParser.tempNetBuffer.hidden << endl;
...
void Debugger::scanForFloatFromSelf(MemReaderInterface* mr_intf, string args)
{
	...
	pStart = mr_intf->extractRAWPointer(offsets[OT_self]);
...
void Debugger::scanForFloat(MemReaderInterface* mr_intf, string args, QWORD pStart, bool yankPstart)
{
	...
	for (pFloat = pStart; pFloat < pEnd; pFloat += 4)
	{
		...
		if (dCheck || tCheck)
		{
			yTemp = mr_intf->extractFloat(pFloat + 4);
		...
		if (tCheck)
		{
			zTemp = mr_intf->extractFloat(pFloat + 8);
		...
		{
			if (xMatch)
				cout << hex << "  X match found at offset 0x" << (pFloat - pStart) << endl;
		}
		...
		if (tCheck)
		{
			...
			{
				if (to_string(round(xTemp * 100.0) / 100.0).substr(0, tokens[0].length()) == to_string(xFind).substr(0, tokens[0].length()) &&
					to_string(round(yTemp * 100.0) / 100.0).substr(0, tokens[1].length()) == to_string(yFind).substr(0, tokens[1].length()) &&
					to_string(round(zTemp * 100.0) / 100.0).substr(0, tokens[2].length()) == to_string(zFind).substr(0, tokens[2].length()))
					cout << hex << "  X,Y,Z match found at offset 0x" << (pFloat - pStart) << ", 0x" << (pFloat - pStart) + 4
						 << ", 0x" << (pFloat - pStart) + 8 << dec << " (" << xTemp << "," << yTemp << "," << zTemp << ")" << endl;
			}
		}
	}
}
...
void Debugger::scanForUINT(MemReaderInterface* mr_intf, QWORD pStart, QWORD size, UINT length, string args)
{
	...
	for (pMem = pStart; pMem < (pStart + size * 2);)
	{
		...
		if (length == 2)
			Temp = mr_intf->extractBYTE(pMem);

		if (Temp == Find)
		{
			cout << dec << Find << "  match found at offset 0x" << hex << (pMem - pStart) << endl;
		}
